...
describe('YoutubeService', () => {
  ...
  describe('extractVideoId', () => {
    it('should extract video ID from youtube.com/watch URL', () => {
      const result = service.extractVideoId('https://www.youtube.com/watch?v=abc123XYZ');
    ...
    it('should extract video ID from youtube.com/embed URL', () => {
      const result = service.extractVideoId('https://www.youtube.com/embed/dEf456Ghi');
      expect(result).toBe('dEf456Ghi');
    });

    it('should extract video ID from URL with additional parameters', () => {
      const result = service.extractVideoId('https://www.youtube.com/watch?v=abc123&list=playlist&t=60');
  ...
  describe('getVideoDetails', () => {
    ...
    it('should return video details on successful API call', async () => {
      ...
      expect(result.description).toBe('Official music video for Never Gonna Give You Up');
    ...
    it('should throw NotFoundException when video not found', async () => {
      ...
      await expect(service.getVideoDetails('invalid-id')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should use medium thumbnail when high is not available', async () => {
      ...
      expect(result.thumbnailUrl).toBe('https://i.ytimg.com/vi/test/mqdefault.jpg');
    });
    ...
    it('should use default thumbnail when high and medium are not available', async () => {
      ...
      expect(result.thumbnailUrl).toBe('https://i.ytimg.com/vi/test/default.jpg');
    });
    ...
    it('should throw NotFoundException on API failure', async () => {
      ...
      await expect(service.getVideoDetails(mockVideoId)).rejects.toThrow(NotFoundException);
    });
  });
});